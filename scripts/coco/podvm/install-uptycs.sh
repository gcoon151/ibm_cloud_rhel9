...
cd /tmp
tar -xzf /tmp/uptycs-complete.tar.gz --exclude='._*'
echo "✓ Extraction complete"
echo ""

# Debug: Show what was extracted
echo "[STEP 5/10] Verifying extracted contents..."
find /tmp/bin /tmp/etc -type f 2>/dev/null | sort || echo "Warning: bin/ or etc/ not found"
echo ""
...
for binary in "${REQUIRED_BINARIES[@]}"; do
    if [ ! -f "/tmp/bin/$binary" ]; then
        echo "✗ ERROR: $binary not found in /tmp/bin/"
        exit 1
    fi
    
    # Copy to /usr/bin/
    cp "/tmp/bin/$binary" "/usr/bin/$binary"
...
rm -rf /tmp/bin /tmp/etc /tmp/VERSION
