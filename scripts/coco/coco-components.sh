...
# Create /etc/containers directory and upload policy.json BEFORE running podvm_maker.sh
# This ensures the file exists when agent-config.toml references it
# Note: --upload places file at exact path, --copy-in copies INTO directory
virt-customize \
    --mkdir /etc/containers \
    --upload $ARTIFACTS_FOLDER/policy.json:/etc/containers/policy.json \
